...
describe("validateUserName", () => {
  ...
  it("should contain only alphanumeric symbols (no spaces either) (returns false and does not call availability check)", async () => {
    await expect(validateUserName("ab#")).resolves.toBe(false);
  ...
  it("should not start with a number (returns false and does not call availability check)", async () => {
    await expect(validateUserName("1abc")).resolves.toBe(false);

    expect(mockedFetch).not.toHaveBeenCalled();
  });

  it("should be unique (returns true when availability check says the name is available)", async () => {
  ...
  it("returns false when availability check says the name is NOT available (the name should be unique)", async () => {
  ...
  it("if fetchIsUserNameAvailable fails (throws an exception), the method returns false", async () => {
